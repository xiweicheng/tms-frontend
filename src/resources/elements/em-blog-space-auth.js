...
export class EmBlogSpaceAuth {
    ...
    attached() {
        ...
        $(this.chk3).checkbox({ // fileReadonly
            onChange: () => {
                if (this._isBlog()) {
                    $.post('/admin/blog/file/permission/update', {
                        id: this.authO.id,
                        readonly: $(this.chk3).checkbox('is checked')
                    }, (data, textStatus, xhr) => {
                        if (data.success) {
                            _.extend(this.authO, data.data);
                            ea.publish(nsCons.EVENT_BLOG_CHANGED, {
                            ...
                            toastr.success('更新博文附件文件下载权限成功!');
                        } else {
                            toastr.error(data.data, '更新博文附件文件下载权限失败!');
                        }
                    });
                }
            }
            ...
            .search({
                ...
                onSelect: (result, response) => {
                    ...
                    if (result.item.username) {
                        _.extend(param, {
                            users: result.item.username
                        });
                    } else {
                        _.extend(param, {
                            channels: result.item.id
                        });
                    }
                ...
                apiSettings: {
                    onResponse: (resp) => {
                        ...
                        $.each(resp.data.channels, (index, item) => {
                            if (!_.find(_.filter(this.shares, c => !c.username), {
                                    name: item.name
                                })) {
                                response.results.push({
                                    item: item,
    ...
    removeShareHandler(item) {
        ...
        if (this._isBlog()) {
            $.post('/admin/blog/auth/remove', param, (data, textStatus, xhr) => {
                if (data.success) {
                    this.shares = _.reject(this.shares, {
                        _id: item._id
                    });
                    this.authO.blogAuthorities = data.data.blogAuthorities;
                } else {
    ...
    showHandler() {
        ...
        $(this.chk3).checkbox(this.authO.fileReadonly ? 'set checked' : 'set unchecked');
        let auths;
        if (this._isBlog()) {
